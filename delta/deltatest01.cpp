...
bool isNumber(const string& str) {
    ...
    for (size_t i = 0; i < str.size(); ++i) {
        if (isdigit(str[i])) {
            ...
            continue; // Allow a leading negative sign
        } else {
...
float applyOperator(float a, float b, const string& op, bool &isValid) {
    ...
    if (op == "**") return pow(a, b); // Power operator
    isValid = false; // Invalid operator
    return 0;
}
...
float evaluateSimpleExpression(vector<string>& tokens, bool &isValid) {
    ...
    for (size_t i = 0; i < operators.size(); ++i) {
        if (operators[i] == "*" || operators[i] == "/") {
            if (newOperands.empty() || i + 1 >= operands.size()) {
                isValid = false;
                return 0;
            }
...
float evaluateExpression(string& expression, bool &isValid) {
    ...
    while ((start = expression.find_last_of('(')) != string::npos) {
        ...
        while (ss >> token) {
            if (!isNumber(token) && token != "+" && token != "-" && token != "*" && token != "/" && token != "**") {
                isValid = false;
                return 0;
            }
        ...
        // Replace the evaluated sub-expression with its result
        expression.replace(start, end - start + 1, to_string(result));
    }

    // After handling all parentheses, evaluate the remaining expression
    vector<string> tokens;
...
// Helper function to insert spaces around operators and parentheses
...
bool isValidExpression(const string& expression) {
    ...
    return openParentheses == 0 && (operators < operands);
}
